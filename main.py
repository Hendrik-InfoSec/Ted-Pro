...
def _build_faq_panel(client_id: str = None) -> str:
    ...
    return (
        ...
        '    +"<div style=\'position:relative\'><input id=\'verify-pw\' type=\'password\' placeholder=\'Enter admin password\' style=\'width:100%;padding:10px 12px;border:0.5px solid #FFD5A5;border-radius:8px;font-size:13px;box-sizing:border-box;margin-bottom:8px;font-family:inherit;padding-right:40px\'>"'
        '+"<button type=\'button\' onclick=\'var v=document.getElementById(\"verify-pw\");if(v.type===\"password\"){v.type=\"text\";this.innerHTML=\"👁️🚫\";this.title=\"Hide password\";}else{v.type=\"password\";this.innerHTML=\"👁️\";this.title=\"Show password\";}\' title=\'Show password\' style=\'position:absolute;right:8px;top:8px;background:none;border:none;cursor:pointer;font-size:18px;color:#8B6914;padding:2px\'>👁️</button></div>"'
...
def _login_page(icon: str, title: str, action: str, error: str = "") -> str:
    ...
    return (
        ...
        '<input type="password" id="pw-field" name="password" placeholder="Password" required '
        'class="w-full px-4 py-3 rounded-xl border border-[#FFD5A5] bg-[#FFF9F4] '
        'focus:outline-none focus:ring-2 focus:ring-[#FF922B] text-sm pr-12">'
        '<button type="button" onclick="togglePw()" id="pw-toggle" '
        'style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:18px;color:#8B6914;padding:4px" '
...
var _cp=new URLSearchParams(location.search).get('client');location.replace('/chat-widget?sid='+encodeURIComponent(s)+(_cp?('&client='+encodeURIComponent(_cp)):''));
    ...
    page = (
        ...
        "    }"
        "    if(d.show_lead&&!window._tedLeadShown&&!window._tedLeadDone){"
        "      window._tedLeadShown=true;"
        "      var lc=document.createElement('div');"
        "      lc.id='widget-lead-form';"
        "      lc.style.cssText='margin:6px 0 8px 0;padding:14px;background:linear-gradient(135deg,#FFF0E0,#FFE4CC);border:1.5px solid #FFD5A5;border-radius:14px';"
        "      lc.innerHTML="
        "        '<div style=\"font-size:13px;font-weight:700;color:#2D1B00;margin-bottom:4px\">&#127873; Want 10% off your first order?</div>'"
        "        +'<div style=\"font-size:12px;color:#5A3A1B;margin-bottom:8px\">Drop your email and I&#39;ll send your discount code straight to your inbox.</div>'"
        "        +'<input id=\"wl-name\" placeholder=\"Your name (optional)\" style=\"width:100%;padding:8px 10px;border:1px solid #FFD5A5;border-radius:8px;font-size:13px;margin-bottom:6px;box-sizing:border-box;font-family:inherit\">'"
        "        +'<input id=\"wl-email\" type=\"email\" placeholder=\"your@email.com\" style=\"width:100%;padding:8px 10px;border:1px solid #FFD5A5;border-radius:8px;font-size:13px;margin-bottom:6px;box-sizing:border-box;font-family:inherit\">'"
        "        +'<button onclick=\"submitWidgetLead()\" style=\"width:100%;padding:9px;background:#FF922B;color:white;border:none;border-radius:8px;font-weight:700;font-size:13px;cursor:pointer;font-family:inherit\">&#129528; Claim my 10% voucher</button>'"
        "        +'<div id=\"wl-msg\" style=\"font-size:11px;color:#991b1b;margin-top:4px;min-height:14px\"></div>';"
        "      msgs.appendChild(lc);scroll();"
        "      var ei=document.getElementById('wl-email');if(ei)ei.onkeydown=function(e){if(e.key==='Enter')submitWidgetLead();};"
        "    }"

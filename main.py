...
from datetime impoimport streamlit as st
from hybrid_engine import HybridEngine
...
st.set_page_config(
    ...
    menu_items={
        'Get Help': 'https://cuddleheros.example.com/support',
        'Report a bug': 'https://cuddleheros.example.com/bugs',
...
def get_leads_df() -> pd.DataFrame:
    ...
    try:
        with get_db_connection() as conn:
            df = pd.read_sql_query("SELECT name, email, context, timestamp FROM leads", conn)
        return df
    except Exception as e:
        logger.error(f"Lead export error: {e}", exc_info=True)
        return pd.DataFrame(columns=["name", "email", "context", "timestamp"])
...
def render_admin_dashboard():
    ...
    if not leads_df.empty:
        ...
        st.download_button(
            ...
            data=csv,
            file_name=f"cuddleheros_leads_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            mime="text/csv"
        )
    else:
    ...
    if not analytics_df.empty:
        ...
        st.download_button(
            ...
            data=csv,
            file_name=f"cuddleheros_analytics_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            mime="text/csv"
        )
    else:
...
# -----------------------------
# UI Styling - Enhanced Performance with Blue/Purple Accents
# -----------------------------
st.markdown("""
<style>
body { 
    background: linear-gradient(180deg, #FFD5A5, #E6E6FA); /* Added purple tint */
    font-family: 'Arial', sans-serif; 
}
.user-msg { 
    background-color: #B0E0E6; /* Light blue for user messages */
    border-radius: 10px; 
    padding: 10px; 
    margin-bottom: 8px; 
    border: 1px solid #4682B4; /* Darker blue border */
    word-wrap: break-word;
    color: #2D1B00;
}
.bot-msg { 
    background-color: #FFF9F4; 
    border-left: 5px solid #800080; /* Purple border for bot */
    border-radius: 10px; 
    padding: 10px; 
    margin-bottom: 8px; 
...
.conversation-scroll { 
    ...
    overflow-y: auto; 
    padding: 10px; 
    border: 1px solid #D8BFD8; /* Light purple */
    border-radius: 10px; 
...
.quick-question-btn { 
    background: linear-gradient(135deg, #4682B4, #800080); /* Blue to purple gradient */
    border: none;
    border-radius: 8px; 
    padding: 10px 6px; 
    cursor: pointer; 
...
.quick-question-btn:hover { 
    background: linear-gradient(135deg, #4169E1, #9932CC); /* Brighter blue to purple */
    transform: translateY(-2px); 
    box-shadow: 0 4px 8px rgba(72, 61, 139, 0.3); /* Slate blue shadow */
}
...
.analytics-badge {
    background: linear-gradient(135deg, #4682B4, #800080); /* Blue to purple */
    color: white;
    padding: 4px 8px;
    border-radius: 12px;
    font-size: 12px;
...
.typing-indicator {
    ...
    padding: 12px 16px;
    background: #FFF9F4;
    border-left: 5px solid #800080; /* Purple */
    border-radius: 10px;
    margin-bottom: 8px;
    border: 1px solid #D8BFD8; /* Light purple */
    font-style: italic;
...
.lead-banner {
    background: linear-gradient(135deg, #E6E6FA, #B0E0E6); /* Purple to blue */
    padding: 20px;
    border-radius: 12px;
    margin: 15px 0;
    text-align: center;
    border: 2px dashed #4682B4; /* Blue */
    animation: pulse 2s infinite;
}
@keyframes pulse {
    0% { border-color: #4682B4; } /* Blue */
    50% { border-color: #800080; } /* Purple */
    100% { border-color: #4682B4; } /* Blue */
}
.checkout-btn {
    background: linear-gradient(135deg, #4682B4, #800080); /* Blue to purple */
    color: white;
...
with col1:
    st.markdown('<h1 style="color:#4682B4;">TedPro Marketing Assistant 🧸</h1>', unsafe_allow_html=True)  # Blue header
with col2:
...
if should_show_lead_banner():
    ...
    <h4 style='color:#800080;margin:0'>🎁 Special Offer for New Friends!</h4>  <!-- Purple header -->
...
def process_message(user_input: str):
    ...
    try:
        ...
        try:
            ...
            first_chunk = True
            friendly_prefix = "Hi there, friend! 🧸 " if is_first else ""
            for chunk in engine.stream_answer(user_input, "en"):
                if first_chunk:
            ...
            if is_purchase_intent:
                filtered_response += f" 💳 You can place your order anytime at [CuddleHeros Store](https://cuddleheros.example.com)."
            bot_placeholder.markdown(
...
voice_input = st.audio_input("Or speak your question!", key="voice_input")
...
<br>
<small style="color: #4682B4;">Professional Plushie Assistant v3.1 - Complete Version</small>
</center>
""", unsafe_allow_html=True)

logger.info("🏁 TedPro application loaded successfully")rt datetime
from pathlib import Path
import json
import os
import re
...
def process_message(user_input: str):
    ...
    try:
        ...
        try:
            ...
            for chunk in engine.stream_answer(user_input, st.session_state.language):
                if first_chunk:
                    raw_response += friendly_prefix + chunk
                    first_chunk = False
                else:
            ...
            filtered_response = raw_response + sales_tail
            if is_purchase_intent:
                filtered_response += f" 💳 {'You can place your order anytime at' if st.session_state.language == 'en' else 'Puedes realizar tu pedido en cualquier momento en'} [Cuddleheroes Store](https://cuddleheroes.example.com)."
            bot_placeholder.markdown(
                f"<div class='bot-msg'><b>TedPro:</b> {filtered_response}"
                f"<div style='font-size:11px;color:#5A3A1B;text-align:right'>{format_timestamp(datetime.now().isoformat())}</div></div>",
                unsafe_allow_html=True
...
<small style="color: #5A3A1B;">© 2025 TedPro Pro Chatbot by Tash & Hendrik 🧸</small>
<br>
<small style="color: #FFA94D;">Professional Plushie Assistant v3.1 - Complete Version</small>
</center>
""", unsafe_allow_html=True)

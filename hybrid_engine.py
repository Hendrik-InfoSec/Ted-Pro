...
class HybridEngine:
        ...
        try:
            ...
            if existing.data and len(existing.data) > 0:
                ...
                self.supabase.table('qa_cache').insert({
                    'question_normalized': normalized, 'answer': answer,
                    'created_at': datetime.now().isoformat(), 'hit_count': 1,
    ...
    def add_lead(self, name: str, email: str, context: str = "chat") -> bool:
        try:
            ...
            self.supabase.table('leads').insert({
                'name': name, 'email': email, 'hashed_email': hashed_email,
                'context': context, 'timestamp': datetime.now().isoformat(),
                'client_id': self.client_id, 'consent': 'yes'
            }).execute()
    ...
    def get_api_answer(self, question: str, stream: bool = True, system_prompt: Optional[str] = None) -> Generator[str, None, None]:
        ...
        headers = {
            ...
            "HTTP-Referer": os.getenv("SITE_URL", "https://tedpro.onrender.com"),
        ...
        for attempt in range(3):
            try:
                response = requests.post(
                    self.api_url, headers=headers, json=payload, timeout=30, stream=stream
                )
